...
- Flake checks execute tests, standard Fourmolu formatting, HLint, package checks and strict documentation builds. Compare the source API against hash-pinned Hackage 0.1.0.1, exercise `onEach` across all Tasty wrappers, and require clean package checks plus complete package Haddocks from the unpacked archive. Embed the current rendered Haddock reference in MkDocs for every library, regardless of Hackage publication, and reject stale files or broken local API links. Deploy to GitHub Pages and verify served guide/API bytes. CI has a Build Gate, named downstream checks and a development-shell build.
