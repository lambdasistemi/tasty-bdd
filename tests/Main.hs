...
main =
    defaultMain $
        testGroup
            ...
            , testCase
                "3 givens and givenandafter order is respected, free language"
                $ do
                    let
            ...
            , testCase "decorators traverse dependent tests" $ do
                let t write =
                        defaultMain $
                            beforeEach (write 0) $
                                testGroup
                                    "dependencies"
                                    [ testCase "dependency" $ write 1
                                    , Tasty.after AllSucceed "dependency" $ testCase "dependent" $ write 2
                                    ]
                testTest t ([0, 1, 0, 2] :: [Int])
            , testCase "decorators preserve failed dependency blocking" $ do
                let t write =
                        defaultMain $
                            beforeEach (write 0) $
                                testGroup
                                    ...
                                    [ testCase "prerequisite" $
                                        write 1 >> assertFailure "expected prerequisite failure"
